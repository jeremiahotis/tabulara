...
- Extended dispatcher state to track derived artifacts and document lifecycle state transitions (`imported` -> `preprocessed` -> `reprocessed`) without mutating prior audit entries.
