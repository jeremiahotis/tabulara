...
test.describe('Story 1.3 document import + duplicate handling workflow (ATDD RED)', () => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await page.getByTestId('import-blob-ids-input').fill('blob-original-001,blob-duplicate-001');
      await page.getByTestId('import-metadata-source-input').fill('import');
      await page.getByTestId('import-file-name-input').fill('invoice-dup-seed.pdf');
      await page.getByTestId('command-submit-button').click();
      await importResponse;
      await expect(page.getByTestId('audit-event-type-latest')).toHaveText('DocumentImported');
      const sourceImportCommandId = (
        await page.getByTestId('audit-event-caused-by-latest').innerText()
      ).trim();
      ...
      await page
        ...
        .fill('session-import-002:blob-duplicate-001');
      await page
        .getByTestId('duplicate-of-document-id-input')
        .fill('session-import-002:blob-original-001');
      await page.getByTestId('duplicate-source-command-id-input').fill(sourceImportCommandId);
      await page.getByTestId('command-submit-button').click();

      await duplicateResponse;

      await expect(page.getByTestId('duplicate-state-latest')).toHaveText('duplicate');
      await expect(page.getByTestId('duplicate-of-document-id-latest')).toHaveText(
        'session-import-002:blob-original-001',
      );
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await page.goto('/');
      await page.getByTestId('command-type-input').fill('ImportDocument');
      await page.getByTestId('import-session-id-input').fill('session-import-003');
      await page.getByTestId('import-blob-ids-input').fill('blob-z,blob-a');
      await page.getByTestId('import-metadata-source-input').fill('import');
      await page.getByTestId('import-file-name-input').fill('invoice-dup-order.pdf');
      await page.getByTestId('command-submit-button').click();
      await importResponse;
      await expect(page.getByTestId('audit-event-type-latest')).toHaveText('DocumentImported');
      const sourceImportCommandId = (
      ...
      const documentId = 'session-import-003:blob-z';
      const duplicateOfDocumentId = 'session-import-003:blob-a';
      const [leftDocumentId, rightDocumentId] = [documentId, duplicateOfDocumentId].sort();
      const expectedDeterministicKey = `session-import-003:${leftDocumentId}:${rightDocumentId}`;
      const duplicateResponse = page.waitForResponse(
        (response) =>
          response.url().includes('/api/v1/commands/dispatch') &&
          response.request().method() === 'POST' &&
      ...
      await page.getByTestId('duplicate-document-id-input').fill(documentId);
      await page.getByTestId('duplicate-of-document-id-input').fill(duplicateOfDocumentId);
      await page.getByTestId('duplicate-source-command-id-input').fill(sourceImportCommandId);
      await page.getByTestId('command-submit-button').click();
      ...
      await expect(page.getByTestId('duplicate-linked-import-command-latest')).toHaveText(
        sourceImportCommandId,
      );
      await expect(page.getByTestId('audit-event-caused-by-latest')).toHaveText(/.+/);
    },
  );
